"""
CompraVenta SV — Property Scraper
====================================
Scrapes real estate listings from compraventasv.com.

CompraVenta is a local Salvadoran marketplace with lower-end
and rural listings that fill coverage gaps in eastern departments.
...
LISTING_PATHS = [
    ("/inmuebles", ""),
    ("/inmuebles/casas", "house"),
    ("/inmuebles/apartamentos", "apartment"),
    ("/inmuebles/terrenos", "land"),
    ("/inmuebles/locales-comerciales", "commercial"),
]

BASE_URL = "https://www.compraventasv.com"


class CompraVentaScraper(BaseScraper):
    """Scrapes property listings from CompraVenta SV."""

    source_name = "compraventa"
    base_url = BASE_URL
    ...
    async def _parse_listing_page(self, html: str, page_url: str, category_type: str) -> list[ScrapedProperty]:
        """Parse a CompraVenta listing page."""
        properties = []
        ...
        try:
            ...
            # Local classifieds sites use varied card layouts
            cards = tree.css(
                ".anuncio, .listing, .ad-item, .product-item, .classified-item, "
                "article, .card, [class*='anuncio'], [class*='listing'], "
                "[class*='result'], [class*='item-product']"
            )

            if not cards:
                cards = tree.css("a[href*='/inmueble'], a[href*='/anuncio'], a[href*='/propiedad'], a[href*='/detalle']")
            ...
            for card in cards:
                try:
                    ...
                    if any(skip in detail_url for skip in [
                        ...
                        "page=", "/categoria", "javascript:", "/buscar"
                    ]):
                        continue

                    title_el = card.css_first("h2, h3, h4, [class*='title'], [class*='nombre'], [class*='titulo']")
                    title = title_el.text(strip=True) if title_el else ""

                    price_el = card.css_first("[class*='precio'], [class*='price'], .price, .precio")
                    ...
                    for spec in card.css("[class*='spec'], [class*='detail'], [class*='caract'], li, span, small"):
                    ...
                    prop = ScrapedProperty(
                        ...
                        price_usd=price_usd,
                        price_raw=price_raw,
                        property_type=category_type or self._guess_type(title + " " + detail_url),
                        bedrooms=bedrooms,
                        bathrooms=bathrooms,
                        area_m2=area_m2,
                        images=images[:5],
            ...
            links = re.findall(
                r'href=["\']([^"\']*(?:/inmueble|/anuncio|/propiedad|/detalle)[^"\']*)["\']',
                html, re.IGNORECASE,
            )
            seen = set()
            for href in links:
    ...
    def _guess_type(self, text: str) -> str:
        text_lower = text.lower()
        for keyword, ptype in [("casa", "house"), ("house", "house"),
    ...
    async def _enrich_from_detail(self, prop: ScrapedProperty) -> ScrapedProperty:
        ...
        if not prop.description:
            desc_match = re.search(
                r'(?:class=["\'][^"\']*(?:descripcion|description|detalle|contenido)[^"\']*["\'][^>]*>)\s*(.*?)\s*</(?:div|p|section)',
                html, re.DOTALL | re.IGNORECASE,
            )
            if desc_match:
    ...
    async def scrape_listings(
        ...
        """Scrape listings from CompraVenta SV."""

        for path, cat_type in LISTING_PATHS:
            logger.info(f"[compraventa] Scraping: {path}")
            ...
            for page_num in range(1, max_pages + 1):
                ...
                logger.info(f"[compraventa] Page {page_num}: {url}")

                resp = await self.fetch(url)
                if not resp:
                    logger.warning(f"[compraventa] Failed page {page_num}, stopping")
                    break
                ...
                if not properties:
                    logger.info(f"[compraventa] No properties on page {page_num}, done with {path}")
                    break
                ...
                logger.info(f"[compraventa] Page {page_num}: {len(properties)} listings")
