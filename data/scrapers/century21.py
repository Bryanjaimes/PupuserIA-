"""
Century 21 El Salvador — Property Scraper
============================================
Scrapes real estate listings from century21.com.sv.

Century 21 is a major international franchise with a strong presence
in El Salvador, particularly for mid-to-high-end residential and
commercial properties.

URL: https://www.century21.com.sv/propiedades
"""
...
BASE_URL = "https://www.century21.com.sv"
...
class Century21Scraper(BaseScraper):
    """Scrapes property listings from Century 21 El Salvador."""
    ...
    async def _parse_listing_page(self, html: str, page_url: str) -> list[ScrapedProperty]:
        """Parse a Century 21 listing page."""
        properties = []
        ...
        try:
            ...
            # Century 21 sites typically use a card-based grid
            cards = tree.css(
                ".listing-card, .property-card, [class*='listing'], "
                "[class*='property-item'], [class*='PropertyCard'], "
                "article, .card, [data-listing], [data-property], "
                ".result-item, [class*='gallery-item']"
            )

            if not cards:
                cards = tree.css(
                    "a[href*='/propiedad/'], a[href*='/listing/'], "
                    "a[href*='/property/'], a[href*='/detalle/']"
                )
            ...
            for card in cards:
                try:
                    ...
                    prop = ScrapedProperty(
                        title=title or f"Century 21 Property in {location or 'El Salvador'}",
                    ...
                    logger.debug(f"Error parsing Century 21 card: {e}")
                    continue

        except ImportError:
            links = re.findall(
                r'href=["\']([^"\']*(?:/propiedad/|/listing/|/property/)[^"\']*)["\']',
                html, re.IGNORECASE,
            )
            for href in set(links):
                url = urljoin(page_url, href)
                prop = ScrapedProperty(
                    title="Century 21 Listing",
                    source=self.source_name,
                    source_url=url,
    ...
    async def _enrich_from_detail(self, prop: ScrapedProperty) -> ScrapedProperty:
        ...
        if not prop.description:
            desc_match = re.search(
                r'(?:class=["\'][^"\']*(?:descripci[oó]n|description|remarks)[^"\']*["\'][^>]*>)\s*(.*?)\s*</(?:div|p|section)',
                html, re.DOTALL | re.IGNORECASE,
            )
            if desc_match:
    ...
    async def scrape_listings(
        ...
        """Scrape listings from Century 21 El Salvador."""
