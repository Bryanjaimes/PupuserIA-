"""
RE/MAX El Salvador — Property Scraper
=======================================
Scrapes real estate listings from remax.com.sv.

RE/MAX is one of the largest international real estate franchises.
Their El Salvador site has well-structured property data, GPS coordinates,
and high-quality listings targeting both local and diaspora buyers.

URL: https://www.remax.com.sv/publiclistinglist.aspx or /propiedades
"""
...
class RemaxScraper(BaseScraper):
    """Scrapes property listings from RE/MAX El Salvador."""
    ...
    async def _parse_listing_page(self, html: str, page_url: str) -> list[ScrapedProperty]:
        ...
        try:
            ...
            # RE/MAX sites typically use listing-card components
            cards = tree.css(
                ".listing-card, .property-card, [class*='listing'], "
                "[class*='property-item'], [class*='PropertyCard'], "
                "[class*='gallery-item'], article, .card, "
                "[data-listing], [data-property], .result-item"
            )

            if not cards:
                # RE/MAX global platform uses specific selectors
                cards = tree.css(
                    "a[href*='/listing/'], a[href*='/propiedad/'], "
                    "a[href*='/property/'], a[href*='MLS'], "
                    "a[href*='/detalle/']"
                )
            ...
            for card in cards:
                try:
                    ...
                    if any(skip in detail_url.lower() for skip in [
                        ...
                        "javascript:", "#", "/buscar", "/news",
                        "/blog", "/about", "/join",
                    ]):
                        continue

                    title_el = card.css_first(
                        "h2, h3, h4, [class*='title'], [class*='address'], "
                        "[class*='listing-name'], [class*='name']"
                    )
                    ...
                    for spec in card.css(
                        "[class*='spec'], [class*='detail'], [class*='feature'], "
                        "[class*='bed'], [class*='bath'], [class*='area'], "
                        "span, li"
                    ):
                    ...
                    for img in card.css("img[src], img[data-src], img[data-original]"):
                        ...
                        if src and not any(s in src.lower() for s in [
                            "placeholder", "logo", "icon", "no-image",
                            "avatar", "agent", "office"
                        ]):
                            images.append(urljoin(page_url, src))

                    prop = ScrapedProperty(
                        title=title or f"RE/MAX Property in {location or 'El Salvador'}",
                    ...
                    logger.debug(f"Error parsing RE/MAX card: {e}")
                    continue

        except ImportError:
            links = re.findall(
                r'href=["\']([^"\']*(?:/listing/|/propiedad/|/property/|MLS)[^"\']*)["\']',
                html, re.IGNORECASE,
            )
            for href in set(links):
                url = urljoin(page_url, href)
                prop = ScrapedProperty(
                    title="RE/MAX Listing",
                    source=self.source_name,
                    source_url=url,
    ...
    async def _enrich_from_detail(self, prop: ScrapedProperty) -> ScrapedProperty:
            ...
            try:
                ...
                if isinstance(ld, list):
                    ld = next((x for x in ld if x.get("@type") in
                              ("Product", "RealEstateListing", "Residence", "House", "Apartment",
                               "SingleFamilyResidence", "ApartmentComplex")), ld[0])
        ...
        if not prop.latitude:
            ...
            if coord_match and lng_match:
                try:
                    ...
                    if 13.0 < lat < 15.0 and -91.0 < lng < -87.0:  # El Salvador bounds
                        prop.latitude = lat
                        prop.longitude = lng
                except ValueError:
                    pass
        ...
        if not prop.description:
            desc_match = re.search(
                r'(?:class=["\'][^"\']*(?:descripci[oó]n|description|listing-description|remarks)[^"\']*["\'][^>]*>)\s*(.*?)\s*</(?:div|p|section)',
                html, re.DOTALL | re.IGNORECASE,
            )
            if desc_match:
    ...
    async def scrape_listings(
        ...
        """Scrape listings from RE/MAX El Salvador."""
        ...
        for path in LISTING_PATHS:
            ...
            # If we got some results from this path, don't try others
            if self._seen_urls:
                break
