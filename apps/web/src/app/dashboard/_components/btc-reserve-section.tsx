...
export function BtcReserveSection({
  ...
  return (
    <section className="border-b border-white/10 py-12 lg:py-16" id="btc-reserve">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-serif text-2xl font-extrabold text-white">{t("dash.btcReserve")}</h2>
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-lg border border-white/10 bg-[#0a0a0a] p-6 transition-all hover:shadow-md">
            ...
            <div className="text-3xl font-extrabold text-white">{btcLoading ? <Skeleton w="w-32" h="h-9" /> : fmtUsd(reserveValue)}</div>
          ...
          <div className="rounded-lg border border-white/10 bg-[#0a0a0a] p-6 transition-all hover:shadow-md">
            ...
            <div className="text-3xl font-extrabold text-white">{btcLoading ? <Skeleton w="w-28" h="h-9" /> : `$${btcPrice?.toLocaleString()}`}</div>
            <div className="mt-1 flex items-center gap-1 text-sm">
              {btcChange24h >= 0
        ...
        <div className="rounded-lg border border-orange-200 bg-orange-50 p-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div><div className="mb-1 text-xs font-medium text-gray-500">{t("dash.firstPurchase")}</div><div className="text-sm font-bold text-white">{BTC_RESERVE.firstBuy}</div><div className="text-[11px] text-gray-400">200 BTC @ ~$46,000</div></div>
            <div><div className="mb-1 text-xs font-medium text-gray-500">{t("dash.managedBy")}</div><div className="text-sm font-bold text-white">{BTC_RESERVE.managedBy}</div><div className="text-[11px] text-gray-400">{BTC_RESERVE.directors}</div></div>
            <div><div className="mb-1 text-xs font-medium text-gray-500">{t("dash.imfStatus")}</div><div className="text-sm font-bold text-white">{BTC_RESERVE.imfStatus}</div><div className="text-[11px] text-gray-400">{BTC_RESERVE.imfLoan}</div></div>
            <div><div className="mb-1 text-xs font-medium text-gray-500">{t("dash.dailyPurchase")}</div><div className="text-sm font-bold text-white">1 BTC / day</div><div className="text-[11px] text-gray-400">Since {BTC_RESERVE.dailyBuySince}</div></div>
          </div>
        </div>
      </div>
    </section>
