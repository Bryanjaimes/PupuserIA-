...
export function Skeleton({ w = "w-20", h = "h-6" }: { w?: string; h?: string }) {
  return <span className={`inline-block ${w} ${h} animate-pulse rounded bg-[#222]`} />;
}
...
export function LiveMetricCard({ dp, loading }: { dp: WBDataPoint | undefined; loading: boolean }) {
  ...
  return (
    <div className={`group relative overflow-hidden rounded-lg border bg-[#0a0a0a] p-4 pl-5 transition-all hover:shadow-md ${isStale ? "border-amber-200" : "border-white/10"}`}>
